...
class MemoryFunctionCallAgent(MemoryMixin, BaseAgent):
    """
...
"""
        ...
        logger.info(f"MemoryFunctionCallAgent '{self.name}' 已初始化")
    ...
    def _get_memory_manager(self, session_id: str) -> MemoryManager:
        """获取会话的记忆管理器（重写Mixin方法以添加日志）"""
        return super()._get_memory_manager(session_id)
    ...
    async def _extract_and_save_memories(
        ...
        """从对话中提取关键信息并保存为记忆"""
        await self._save_conversation_memory(user_input, assistant_response, memory_manager)
